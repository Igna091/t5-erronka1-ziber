...
<div class="page-header">
    <div class="page-header__title">
        <h1 class="h-admin"><span class="fx-type">Panel.<span class="cursor" aria-hidden="true"></span></span></h1>
        <span class="text-2">Resumen del centro</span>
    </div>
</div>
...
<div class="stats-grid">
    <div class="stat fx-fade" style="--d: 0.1s">
        ...
        <span class="stat__sub"><span class="acc">{{ $stats['registered_students'] }}</span> {{ $stats['registered_students'] === 1 ? 'activado' : 'activados' }} · <span class="warn">{{ $stats['pending_students'] }}</span> {{ $stats['pending_students'] === 1 ? 'pendiente' : 'pendientes' }}</span>
    </div>
    <div class="stat fx-fade" style="--d: 0.18s">
        ...
        @endphp
        <span class="stat__sub">{{ $stats['active_courses'] }} {{ $stats['active_courses'] === 1 ? 'activo' : 'activos' }} · {{ $inactiveCourses }} {{ $inactiveCourses === 1 ? 'inactivo' : 'inactivos' }}</span>
    </div>
    <div class="stat fx-fade" style="--d: 0.26s">
        <span class="lbl">matrículas activas</span>
        <span class="stat__value">{{ $stats['total_enrollments'] }}</span>
        <span class="stat__sub">{{ $stats['cancelled_enrollments'] }} {{ $stats['cancelled_enrollments'] === 1 ? 'cancelada' : 'canceladas' }}</span>
    </div>
    <div class="stat stat--acc fx-fade" style="--d: 0.34s">
        <span class="lbl">plazas libres</span>
        <span class="stat__value">{{ $stats['free_seats'] }}</span>
        <span class="stat__sub">de {{ $stats['total_seats'] }} en cursos activos</span>
    </div>
</div>

<div class="grid-2">
    <section class="panel fx-fade" style="--d: 0.4s">
        <div class="panel__head"><h2 class="lbl">alumnos recientes</h2><a href="{{ route('admin.students.index') }}" class="link-arrow small">ver todos</a></div>
        ...
        @empty
            <div class="empty"><span class="empty__cmd">ls alumnos/ <span class="muted">— vacío</span></span></div>
        @endforelse
    </section>

    <section class="panel fx-fade" style="--d: 0.48s">
        <div class="panel__head"><h2 class="lbl">matrículas recientes</h2><a href="{{ route('admin.enrollments.index') }}" class="link-arrow small">ver todas</a></div>
        @forelse ($recentEnrollments as $enrollment)
            <a href="{{ $enrollment->student ? route('admin.students.show', $enrollment->student) : '#' }}" class="row-link" style="grid-template-columns: minmax(0, 1fr) 5.5rem 7rem;">
                ...
                <span class="xs muted col-when" style="text-align: right;">{{ $enrollment->created_at->locale('es')->diffForHumans() }}</span>
            </a>
        @empty
            <div class="empty"><span class="empty__cmd">ls matrículas/ <span class="muted">— vacío</span></span></div>
        @endforelse
    </section>
</div>

<section class="panel fx-fade" style="--d: 0.56s">
    <div class="panel__head"><h2 class="lbl">ocupación de cursos</h2><a href="{{ route('admin.courses.index') }}" class="link-arrow small">gestionar cursos</a></div>
    <div class="occupancy">
        @forelse ($courses as $course)
            ...
            <div class="ls__row {{ $course->isActive() ? '' : 'is-off' }}">
                ...
                <span class="xs col-status {{ $course->isActive() ? 'text-2' : 'muted' }}" style="text-align: right;">{{ $course->isActive() ? 'activo' : 'inactivo' }}</span>
            </div>
        @empty
            <div class="empty"><span class="empty__cmd">ls cursos/ <span class="muted">— vacío</span></span></div>
        @endforelse
    </div>
</section>
@endsection
