...
<div class="form-row">
    <div class="form-group">
        ...
        <input type="text" id="name" name="name" class="form-input @error('name') is-invalid @enderror" value="{{ old('name', $course->name ?? '') }}" required maxlength="100" placeholder="Desarrollo Web Full Stack">
        @error('name')<span class="form-error">{{ $message }}</span>@enderror
    </div>
    <div class="form-group">
        <label for="code" class="form-label">código <span class="req">*</span></label>
...
<div class="form-group">
    ...
    <textarea id="description" name="description" class="form-textarea @error('description') is-invalid @enderror" maxlength="5000" placeholder="Qué se aprende en el curso…">{{ old('description', $course->description ?? '') }}</textarea>
...
<div class="form-row">
    ...
    <div class="form-group">
        ...
        <input type="number" id="capacity" name="capacity" class="form-input @error('capacity') is-invalid @enderror" value="{{ old('capacity', $course->capacity ?? '') }}" min="1" max="1000" placeholder="vacío = sin límite" aria-describedby="capacity-help">
        <span class="form-help" id="capacity-help">Déjalo vacío si no hay límite de plazas.</span>
...
<div class="form-row">
    <div class="form-group">
        ...
        <span class="form-help" id="start-help">El año académico se calcula con esta fecha (empieza en septiembre).</span>
...
<div class="form-group" style="max-width: calc(50% - 0.625rem); min-width: min(100%, 16rem);">
    <label for="status" class="form-label">estado <span class="req">*</span></label>
    <select id="status" name="status" class="form-select @error('status') is-invalid @enderror">
        <option value="active" @selected(old('status', $course->status ?? 'active') === 'active')>activo · admite matrículas</option>
        <option value="inactive" @selected(old('status', $course->status ?? 'active') === 'inactive')>inactivo · oculto al público</option>
    </select>
